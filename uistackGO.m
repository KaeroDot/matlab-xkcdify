function uistackGO(comp, moveto, step)
% uistackGO Reorder visual stacking of graphics objects
% (written only to reimplement Matlab's uistack function for GNU Octave)
%
% Syntax:
%   uistackGO(comp)
%   uistackGO(comp, moveto)
%   uistackGO(comp, moveto, step)
%
% Description:
%   uistackGO(comp) shifts the specified graphics object(s) up one level 
%   within the visual stacking order. If comp is a vector of objects, 
%   uistackGO shifts each object in the vector up one level.
%
%   uistackGO(comp, moveto) moves the graphics object to the specified 
...
%
%   uistackGO(comp, moveto, step) specifies the number of levels to move
...
% See also: set, get
  ...
  for i = 1:numel(comp)
    ...
    parent = get(h, 'Parent');
    if isempty(parent)
      warning('uistackGO: object has no parent, cannot reorder');
      continue;
    endif
    ...
    if isempty(current_idx)
      warning('uistackGO: object not found in parent''s children');
      continue;
    endif
    ...
    switch moveto
      ...
      otherwise
        error('uistackGO: invalid moveto value');
    endswitch
...
%! % Bring red rectangle to front
%! uistackGO(red, 'top');
%! title('After uistackGO(r1, ''top''): red on top (and blue in back, green in middle)');
%! children = get(gca, 'Children')
%! input('Press Enter to continue...');
%! 
%! % Move red down one level
%! uistackGO(red, 'down', 1);
%! children = get(gca, 'Children')
%! title('After uistackGO(r1, ''down'', 1): red in middle');
